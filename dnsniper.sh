...
setup_systemd_service() {
    log "INFO" "Setting up systemd service and timer" "verbose"
...
EOF
    ...
    # Create firewall rules persistence service
...
EOF
    ...
    systemctl enable dnsniper-firewall.service 2>/dev/null || true
    
    log "INFO" "DNSniper systemd services created" "verbose"
    
    # If asked to use systemd timer instead of cron
    local use_systemd=$(grep '^use_systemd=' "$CONFIG_FILE" 2>/dev/null | cut -d= -f2 || echo "0")
    if [[ "$use_systemd" == "1" ]]; then
        # Disable cron job if exists
        crontab -l 2>/dev/null | grep -v "$BIN_CMD" | crontab - 2>/dev/null || true
        # Enable systemd timer
        systemctl enable dnsniper.timer 2>/dev/null || true
        systemctl start dnsniper.timer 2>/dev/null || true
        log "INFO" "DNSniper systemd timer enabled" "verbose"
    fi
}
...
make_rules_persistent() {
    ...
    # Ensure our systemd service exists for persistence
...
EOF
    ...
    if [[ ${#missing_deps[@]} -gt 0 ]]; then
        log "WARNING" "Missing dependencies: ${missing_deps[*]}" "verbose"
        echo "Warning: The following commands are not installed: ${missing_deps[*]}" >&2
        echo "Some features may not work properly. Please install them using your system's package manager." >&2
    fi
...
SQL
    ...
    # Install cron job if defined in config and systemd not configured
    local use_systemd=$(grep '^use_systemd=' "$CONFIG_FILE" 2>/dev/null | cut -d= -f2 || echo "0")
    if [[ "$use_systemd" != "1" ]]; then
        ...
        if [[ -n "$cron_expr" && "$cron_expr" != "# DNSniper disabled" ]]; then
            ...
            log "INFO" "Cron job installed: $cron_expr" "verbose"
        fi
    else
        # Remove any existing cron jobs when using systemd
        crontab -l 2>/dev/null | grep -v "$BIN_CMD" | crontab - 2>/dev/null || true
        log "INFO" "Using systemd timer instead of cron" "verbose"
    fi
...
settings_menu() {
    while true; do
        ...
        echo_safe "${YELLOW}A.${NC} Toggle Systemd/Cron"
        ...
        case "$choice" in
            ...
            [Aa]) toggle_systemd ;;
            0) return ;;
            *) echo_safe "${RED}Invalid selection. Please choose 0-9 or A.${NC}" ;;
        esac
        read -rp "Press Enter to continue..."
    done
}

# Toggle between systemd and cron
toggle_systemd() {
    echo_safe "${BOLD}=== Toggle Systemd/Cron ===${NC}"
    
    # Get current setting
    local use_systemd=$(grep '^use_systemd=' "$CONFIG_FILE" 2>/dev/null | cut -d= -f2 || echo "0")
    
    if [[ "$use_systemd" == "1" ]]; then
        echo_safe "${BLUE}Current scheduler:${NC} ${GREEN}Systemd${NC}"
        read -rp "Switch to cron job scheduling? [y/N]: " choice
        ...
        if [[ "$choice" =~ ^[Yy] ]]; then
            ...
            # Update config
            sed -i "s|^use_systemd=.*|use_systemd=0|" "$CONFIG_FILE"
            ...
            echo_safe "${YELLOW}Switched to cron job scheduling.${NC}"
            log "INFO" "Switched to cron job scheduling" "verbose"
        else
            echo_safe "${YELLOW}No change.${NC}"
        fi
    else
        echo_safe "${BLUE}Current scheduler:${NC} ${YELLOW}Cron${NC}"
        
        # Check if systemd is available
        if ! command -v systemctl &>/dev/null; then
            echo_safe "${RED}Systemd not available on this system.${NC}"
            return 1
        fi
        
        read -rp "Switch to systemd timer scheduling? [y/N]: " choice
        ...
        if [[ "$choice" =~ ^[Yy] ]]; then
            ...
            # Update config
            sed -i "s|^use_systemd=.*|use_systemd=1|" "$CONFIG_FILE"
            ...
            echo_safe "${GREEN}Switched to systemd timer scheduling.${NC}"
            log "INFO" "Switched to systemd timer scheduling" "verbose"
        else
            echo_safe "${YELLOW}No change.${NC}"
        fi
    fi
...
toggle_logging() {
    ...
    if [[ $LOGGING_ENABLED -eq 1 ]]; then
        ...
        # Offer to set log rotation settings
        read -rp "Adjust log rotation settings? [y/N]: " rotate_choice
        if [[ "$rotate_choice" =~ ^[Yy] ]]; then
            local max_size=$(grep '^log_max_size=' "$CONFIG_FILE" 2>/dev/null | cut -d= -f2 || echo "$DEFAULT_LOG_MAX_SIZE")
            local rotate_count=$(grep '^log_rotate_count=' "$CONFIG_FILE" 2>/dev/null | cut -d= -f2 || echo "$DEFAULT_LOG_ROTATE_COUNT")
            
            echo_safe "${BLUE}Current log size limit:${NC} ${YELLOW}${max_size}MB${NC}"
            read -rp "Enter new log size limit in MB (1-100): " new_size
            
            if [[ "$new_size" =~ ^[0-9]+$ && $new_size -ge 1 && $new_size -le 100 ]]; then
                sed -i "s|^log_max_size=.*|log_max_size=$new_size|" "$CONFIG_FILE"
                echo_safe "${GREEN}Log size limit set to ${new_size}MB.${NC}"
            else
                echo_safe "${RED}Invalid input. Using current value.${NC}"
            fi
            
            echo_safe "${BLUE}Current log rotation count:${NC} ${YELLOW}${rotate_count} files${NC}"
            read -rp "Enter new log rotation count (1-20): " new_count
            
            if [[ "$new_count" =~ ^[0-9]+$ && $new_count -ge 1 && $new_count -le 20 ]]; then
                sed -i "s|^log_rotate_count=.*|log_rotate_count=$new_count|" "$CONFIG_FILE"
                echo_safe "${GREEN}Log rotation count set to ${new_count}.${NC}"
            else
                echo_safe "${RED}Invalid input. Using current value.${NC}"
            fi
        fi
...
set_schedule() {
    ...
    # Display appropriate current schedule
    if [[ "$use_systemd" == "1" ]]; then
        echo_safe "${BLUE}Current schedule:${NC} Using systemd timer (hourly)"
        echo_safe "${YELLOW}Note:${NC} To change to cron scheduling, use the Toggle Systemd/Cron option first."
        return 0
    else
        echo_safe "${BLUE}Current schedule:${NC} $cron_expr"
    fi
...
import_all() {
    ...
    if [[ -f "$dir/domains.txt" || -f "$dir/ips.txt" || -f "$dir/config.conf" || -f "$dir/history.db" ]]; then
        ...
        if [[ -f "$dir/history.db" && -r "$dir/history.db" ]]; then
            # Stop any running processes before database restoration
            for pid in $(pgrep -f "$BIN_CMD" | grep -v "$$"); do
...
display_status() {
    ...
    (
        ...
        local schedule_text
        if [[ "$use_systemd" == "1" ]]; then
            schedule_text="${GREEN}Systemd timer (hourly)${NC}"
        else
            schedule_text="$sched"
...
uninstall() {
    ...
    if [[ "$confirm" =~ ^[Yy] ]]; then
        ...
        # Remove systemd service and timer if we created it
        if command -v systemctl &>/dev/null; then
            systemctl stop dnsniper.timer 2>/dev/null || true
            systemctl disable dnsniper.timer 2>/dev/null || true
        ...
        # Remove cron job
        crontab -l 2>/dev/null | grep -vF "$BIN_CMD" | crontab - 2>/dev/null || true
